...
- [ ] Phase 10. ペイン整合性監視機能（要件 5.8 対応）
  - TDD-10.1: tmuxio拡張 - `list_panes_with_titles()` メソッド追加
    ...
    - FakeIO: テスト用モック実装
  - TDD-10.2: Orchestrator拡張 - `check_pane_integrity()` メソッド追加
    - 期待セッション名の集合とペインタイトルの集合を比較
    - 不一致検出時に `_last_signature = None` で再レイアウト強制
    - ログ出力（警告レベル）での通知
  - TDD-10.3: 統合テスト - ペイン削除シミュレーション
    - 正常時: ペインタイトルが全て一致
    - 異常時: ペイン削除後の整合性チェックと自動復旧
    - エッジケース: ペイン数は同じだが内容が異なる場合
  - TDD-10.4: E2E検証（実tmux環境）
    - dashboardでペインを手動削除 → 次回ポーリングで自動復旧
    - 復旧後の状態確認（タイトル、レイアウト）
  - 実装: tmuxio.py, orchestrator.py の修正
  - フェーズ終了検証: テスト全グリーン → 提出  
  - _要件: 5.8_
...
| ペイン整合性の崩れ | 中 | タイトル監視による検出、自動再レイアウトによる復旧、2秒周期での早期検出 |
