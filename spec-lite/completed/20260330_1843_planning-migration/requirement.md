...
### 5.8 dashboardペイン整合性監視
- **ペインタイトル監視**：各ポーリングサイクルで dashboard セッション内の全ペインタイトルを取得。
  - `tmux list-panes -t dashboard:0 -F "#{pane_title}"` でタイトル一覧を取得。
  - 各タイトルは対応するセッション名を示す。
- **整合性チェック**：
  - 取得したペインタイトルのセットと、表示すべきセッション名のセット（除外後）を比較。
  - ペイン数の一致だけでなく、内容の一致も検証。
- **不整合時の処理**：
  - ペインタイトルが期待値と異なる場合、全面再レイアウトを実行。
  - `kill-pane -a` でリセット後、改めて必要なペインを作成。
  - タイトル設定とレンダラー起動を再実行。
- **発生シナリオ**：
  - ユーザーの操作ミスによる dashboard 内ペインの削除（Ctrl-C等）。
  - ユーザーが誤ってペインを手動作成した場合。
  - 頻繁ではないが、発生時は自動的に修復。
- **検出タイミング**：セッション監視・ウィンドウサイズ監視と同一サイクル内。
- **パフォーマンス考慮**：
  - ペインタイトル取得は単一tmuxコマンドで完了（追加負荷は最小限）。
  - 文字列セットの比較は高速（O(n)、nはセッション数）。
...
10. **ペイン整合性**：dashboard 内のペインが手動削除・追加された場合、3秒以内に正しいセッションに対応するペインが再構築される。
...
| dashboard内ペイン手動削除 | レイアウト不整合 | 各サイクルでペイン数を監視、不一致時は全面再レイアウトで自動修復。 |
